...
export function TransactionForm({
  ...
  const [transactionType, setTransactionType] = useState<'debit' | 'credit'>(
    transaction?.debit && parseFloat(transaction.debit) > 0 ? 'debit' : 'credit'
  );
  ...
  const addTransactionMutation = trpc.addTransaction.useMutation({
    ...
    onError: (error) => {
      ...
      setErrors({
        general: "Une erreur est survenue lors de l'ajout",
      });
    },
  });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!notes.trim()) {
      newErrors.notes = "La description est requise";
    }
    ...
    if (transactionType === 'debit') {
      debitValue = parseFloat(debit) || 0;
      if (debitValue <= 0) {
        newErrors.amount = "Veuillez saisir un montant valide pour le débit";
      }
    } else {
      creditValue = parseFloat(credit) || 0;
      if (creditValue <= 0) {
        newErrors.amount = "Veuillez saisir un montant valide pour le crédit";
      }
    }
                ...
                <div className="mb-5">
                  ...
                  <div className={cn(
                    ...
                    <Input
                      ...
                      min="0"
                      step="0.01"
                      value={transactionType === 'debit' ? debit : credit}
                      onChange={(e) => {
                        if (transactionType === 'debit') {
                      ...
                      className={cn(
                        ...
                        transactionType === 'debit' ? "text-red-600 dark:text-red-400" : "text-emerald-600 dark:text-emerald-400",
                ...
                <div className="mb-3">
                  ...
                  <div className="flex items-center rounded-lg h-11 overflow-hidden transition-all duration-200 bg-white dark:bg-slate-800 ring-1 ring-gray-200 dark:ring-slate-700 border-transparent focus-within:ring-blue-300 dark:focus-within:ring-blue-500/50 shadow-sm">
                    ...
                    <Input
                      id="date"
                      type="date"
                      value={isValidDate ? format(date, "yyyy-MM-dd") : ""}
                      onChange={(e) => {
                        try {
